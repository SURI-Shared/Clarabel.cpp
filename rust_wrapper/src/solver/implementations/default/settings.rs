...
/// Get the default settings for the solver
#[allow(non_snake_case)]
fn _internal_DefaultSettings_rerunnable<T: FloatT>() -> ClarabelDefaultSettings<T> {
    let mut default = clarabel::solver::DefaultSettings::<T>::default();
    default.verbose=false;
    default.presolve_enable=false;
    let default_direct_solver_setting = match default.direct_solve_method.as_str() {
        "qdldl" => ClarabelDirectSolveMethods::QDLDL,
        //"mkl" => ClarabelDirectSolveMethods::MKL,
        //"cholmod" => ClarabelDirectSolveMethods::CHOLMOD,
...
#[no_mangle]
pub extern "C" fn clarabel_DefaultSettings_f64_rerunnable() -> ClarabelDefaultSettings<f64> {
    _internal_DefaultSettings_rerunnable::<f64>()
}
